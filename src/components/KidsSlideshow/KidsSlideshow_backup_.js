...
const KidsSlideshow = ({ data }) => {
    ...
    const {
        ...
        markComplete,
        addAttempt
    } = useProgressV2();    // State để lưu currentSlideData
    const [currentSlideData, setCurrentSlideData] = useState({});

    // Effect để merge quiz/exercise data khi slide thay đổi
    useEffect(() => {
        ...
        // Merge quiz data
        if (slideData.type === 'quiz' && slideData.quizId && quizzes) {
            ...
            if (quizQuestions.length > 0) {
                console.log('🔀 Merged quiz questions:', quizQuestions);
                setCurrentSlideData({ ...slideData, questions: quizQuestions });
                return;
            }
        }

        // Merge exercise data  
        if (slideData.type === 'exercise' && slideData.exerciseId && exercises) {
            const exerciseQuestions = exercises.filter(e =>
                ...
                console.log('🔀 Merged exercise questions:', exerciseQuestions);
                setCurrentSlideData({ ...slideData, questions: exerciseQuestions });
                return;
            }
        }

        // Nếu không cần merge, set slide data thông thường        setCurrentSlideData(slideData);
    }, [slides, currentSlide, quizzes, exercises]);

    // Effect để cập nhật Redux khi slide thay đổi
    useEffect(() => {
        if (currentSlideData && currentSlideData.id) {
            console.log('Updating Redux with slide:', currentSlideData.title);
            setSlide({
